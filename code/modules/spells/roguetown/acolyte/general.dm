...
/obj/effect/proc_holder/spell/invoked/lesser_heal/cast(list/targets, mob/living/user)
	...
	if(isliving(targets[1]))
		...
		switch(user.patron.type)
			...
			if(/datum/patron/draconic/anarinus)
				target.visible_message(span_info("Golden flames limn the body of [target] and the world feels brighter!"), span_notice("The promise of a brighter day sends my soul soaring!"))
			if(/datum/patron/draconic/moonari)
				target.visible_message(span_info("Silver flames limn the body of [target] and the world comes into sharp clarity!"), span_notice("The promise of secrets to be uncovered rouses my spirit!"))
			if(/datum/patron/draconic/deemimoria)
				target.visible_message(span_info("Green flames limn the body of [target] and the world seems to liven up!"), span_notice("The promise of abundant bounty makes my heart feel full!"))
			if(/datum/patron/draconic/augwinia)
				target.visible_message(span_info("Blue flames limn the body of [target] and the world crashes into motion!"), span_notice("The promise of action to come sends fresh blood rushing through me!"))
			if(/datum/patron/draconic/revillicus)
			...
			if(/datum/patron/draconic/magnus)
				target.visible_message(span_info("Smoking flames limn the body of [target] as a rush of wild inspiration is released into the world!"), span_notice("The promise of a world full of wonders urges me on!"))
			if(/datum/patron/draconic/onissia)
				target.visible_message(span_info("Pink flames limn the body of [target] and the world seems more peaceful, momentarily!"), span_notice("The promise of love and warmth fills my stomach with butterflies!"))
			else
